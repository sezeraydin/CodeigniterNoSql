...
class Selam extends CI_Controller {


	public function index(){

		if(FIRELOG) flog("info","Controller 'Selam' constructed");
		...
		$data['title'] = 'sadece bir deneme';
		$data['bold'] = true;
		$data['ip_address'] = $this->input->server('REMOTE_ADDR');

		// Calling the convenience function view() that allows passing data
		//$this->smarty->view( 'mainpage.tpl', $data );
		$this->smarty->assign("demo","");
		$this->smarty->view( 'girisdenem.tpl', $data );
	}

	function reklam(){
		$this->smarty->view( 'oynareklamkayitA.php');
	}
	
    function reklam2(){
		$this->smarty->view( 'oynareklamkayitB.php');
	}
}
